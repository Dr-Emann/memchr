...
fn main() -> anyhow::Result<()> {
    ...
    let samples = match (&*engine, &*b.model) {
        ...
        ("range-scalar", "count-bytes") => range_count(&b, Backend::Swar)?,
...
fn memchr_prebuilt_count(
    ...
    let haystack = &b.haystack;
    let needle = b.one_needle_byte()?;
    let finder = MemchrN::new_with_backend(&[needle], backend);
...
fn memchr2_count(
    ...
    let finder = MemchrN::new_with_backend(&[n1, n2], backend);
    shared::run(b, || Ok(finder.iter(haystack).count_slow()))
}
...
fn memchr3_count(
    ...
    let finder = MemchrN::new_with_backend(&[n1, n2, n3], backend);
    shared::run(b, || Ok(finder.iter(haystack).count_slow()))
}
...
fn byteset_build_count(b: &Benchmark) -> anyhow::Result<Vec<Sample>> {
    let haystack = &b.haystack;
    let bytes = needle_bytes(b)?;
    shared::run(b, || Ok(MemchrN::new(&bytes).iter(haystack).count_slow()))
}
